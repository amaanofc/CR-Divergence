...
export default function BacktestReport() {
  ...
  if (data.warning) {
    return (
      <div className="p-4">
        <div className="border border-yellow-700 text-yellow-400 text-sm p-3 mb-3">
          ⚠ {data.warning}
        </div>
      </div>
    )
  ...
  return (
    <div className="p-3 h-full flex flex-col gap-3 overflow-y-auto">
      ...
      {/* Stats table */}
      <div>
        <div className="text-bloomberg-muted text-xs mb-1">Strategy Performance Summary</div>
        <table className="w-full text-xs">
          <thead>
            <tr className="text-bloomberg-muted border-b border-bloomberg-border">
              ...
              <th className="text-right py-0.5">Total Excess WR</th>
            ...
            {Object.entries(strategies).map(([key, strat]) => {
              ...
              return (
                <tr key={key} className="border-b border-bloomberg-border">
                  ...
                  <td className="py-0.5 text-right">{strat.sharpe?.toFixed(3) ?? '—'}</td>
                  <td className="py-0.5 text-right text-red-400">
                    {strat.max_drawdown != null ? `${(strat.max_drawdown * 100).toFixed(1)}%` : '—'}
                  </td>
                  <td className="py-0.5 text-right">
                    {strat.total_excess_win_rate?.toFixed(3) ?? '—'}
                  </td>
                  <td className={`py-0.5 text-right ${finalCA > 0 ? 'text-green-400' : 'text-red-400'}`}>
